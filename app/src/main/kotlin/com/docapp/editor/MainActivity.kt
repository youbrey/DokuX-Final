...
class MainActivity : ComponentActivity() {
    ...
    private val requestAudioPermissionLauncher = registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted ->
        if (isGranted) {
            ...
            Toast.makeText(this, "Izin mikrofon diperlukan untuk fitur Suara ke Teks", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun generateWithAi(prompt: String) = lifecycleScope.launch {
        ...
        if (account == null || account.idToken.isEmpty()) {
            Toast.makeText(
                this@MainActivity,
                "Anda harus login ke akun Google terlebih dahulu untuk menggunakan fitur AI.",
                Toast.LENGTH_LONG
            ).show()
            requestGoogleSignIn()
            return@launch
        ...
        when (val result = aiGenerator.generate(prompt, account.idToken)) {
            is AiGenerateResult.Success -> {
                viewModel.loadDocument(result.document)
                Toast.makeText(this@MainActivity, "Dokumen AI berhasil dibuat!", Toast.LENGTH_SHORT).show()
            }
            is AiGenerateResult.Error -> {
                Toast.makeText(this@MainActivity, "Gagal membuat dokumen: ${result.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun startVoiceInput() {
        ...
        if (account == null) {
            Toast.makeText(
                this@MainActivity,
                "Anda harus login ke akun Google terlebih dahulu untuk menggunakan fitur Suara ke Teks.",
                Toast.LENGTH_LONG
            ).show()
            requestGoogleSignIn()
            return
    ...
    private fun startVoiceInputInternal() = lifecycleScope.launch {
        Toast.makeText(this@MainActivity, "Mendengarkan suara... Silakan bicara", Toast.LENGTH_SHORT).show()
        micTranscriber.listen().collectLatest { result ->
            when (result) {
                is TranscriptionResult.Success -> {
                    viewModel.insertTranscribedText(viewModel.focusedParagraph.value, result.text)
                    Toast.makeText(this@MainActivity, "Teks suara ditambahkan!", Toast.LENGTH_SHORT).show()
                }
                is TranscriptionResult.Error -> Toast.makeText(this@MainActivity, "Suara: ${result.message}", Toast.LENGTH_SHORT).show()
